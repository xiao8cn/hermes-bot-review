...
export default function Home() {
  ...
  useEffect(() => {
    if (!data || !sessionTestResults) return;

    const agentSessionCount = new Map<string, number>();
    for (const agent of data.agents) {
      agentSessionCount.set(agent.id, agent.session?.sessionCount || 0);
    }

    let changed = false;
    const next: Record<string, { ok: boolean; reply?: string; error?: string; elapsed: number } | null> = {};

    for (const [agentId, result] of Object.entries(sessionTestResults)) {
      if (!agentSessionCount.has(agentId)) {
        changed = true;
        continue;
      }

      if (result && !result.ok && /no sessions found/i.test(result.error || "")) {
        const count = agentSessionCount.get(agentId) || 0;
        if (count > 0) {
          next[agentId] = { ok: true, reply: `Found ${count} sessions`, elapsed: 0 };
          changed = true;
          continue;
        }
      }
    ...
    if (changed) {
      setSessionTestResults(next);
    }
  }, [data, sessionTestResults]);
      ...
      fetch("/api/agent-activity", { cache: "no-store" })
        ...
        .then(d => {
          if (d.agents) {
            ...
            for (const s of d.agents) map[s.agentId] = s.state;
            setAgentStates(map);
  ...
  const getEffectiveSessionTestResult = (agentId: string, sessionCount?: number) => {
    const raw = sessionTestResults?.[agentId];
    if (!raw) return raw;
    if (!raw.ok && /no sessions found/i.test(raw.error || "") && (sessionCount || 0) > 0) {
      return { ok: true, reply: `Found ${sessionCount} sessions`, elapsed: 0 };
    }
    return raw;
  };
    ...
    <div className="p-3 md:p-4 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {data.agents.map((agent) => (
          <AgentCard key={agent.id} agent={agent} gatewayPort={data.gateway?.port || 18789} gatewayToken={data.gateway?.token} gatewayHost={data.gateway?.host} t={t} testResult={testResults?.[agent.id]} platformTestResults={platformTestResults || undefined} sessionTestResult={getEffectiveSessionTestResult(agent.id, agent.session?.sessionCount)} agentState={agentStates[agent.id]} dmSessionResults={dmSessionResults || undefined} providerAccessModeMap={providerAccessModeMap} modelOptions={modelOptions} onModelChange={changeAgentModel} />
        ))}
      </div>
