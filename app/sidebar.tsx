...
type NavIconName = "agents" | "pixelOffice" | "models" | "sessions" | "stats" | "skills";
...
function HermesLogo({ className, size = 72 }: { className?: string; size?: number }) {
  return (
    <img
      src="/assets/branding/hermes-logo.png"
      alt="Hermes"
      className={className}
      draggable={false}
...
export function Sidebar() {
  ...
  const [mobileHermesVersion, setMobileHermesVersion] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    window.addEventListener("hermes-logo-drag-start", onStart as EventListener);
    window.addEventListener("hermes-logo-drag-stop", onStop as EventListener);
    window.addEventListener("hermes-logo-carry-progress", onProgress as EventListener);
    return () => {
      window.removeEventListener("hermes-logo-drag-start", onStart as EventListener);
      window.removeEventListener("hermes-logo-drag-stop", onStop as EventListener);
      window.removeEventListener("hermes-logo-carry-progress", onProgress as EventListener);
    };
  }, []);
  ...
  useEffect(() => {
    let cancelled = false;
    const fetchHermesVersion = async () => {
      try {
        const res = await fetch("/api/gateway-health", { cache: "no-store" });
        if (!res.ok) return;
        const data = await res.json();
        if (cancelled) return;
        const version = typeof data?.hermesVersion === "string" ? data.hermesVersion.trim() : "";
        setMobileHermesVersion(version || null);
      } catch {}
    };
    void fetchHermesVersion();
    return () => {
      cancelled = true;
    };
  }, []);
            ...
            >
              ...
              <div className="min-w-0">
                <div className="text-xs font-bold tracking-wide truncate">
                  HERMES{mobileHermesVersion ? ` ${mobileHermesVersion}` : ""}
                </div>
            ...
            <div>
              <div className="flex items-center justify-between">
                ...
                >
                  ...
                  <div>
                    <div className="text-sm font-bold text-[var(--text)] tracking-wide">HERMES</div>
