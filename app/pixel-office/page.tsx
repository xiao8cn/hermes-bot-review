...
export default function PixelOfficePage() {
  ...
  useEffect(() => {
    window.dispatchEvent(new CustomEvent('hermes-logo-drag-start'))
    return () => {
      window.dispatchEvent(new CustomEvent('hermes-logo-drag-stop'))
    }
  }, [])
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if (editor.isEditMode) {
      ...
      const catNpcId = office.getFirstLobsterAt(worldX, worldY)
      ...
      if (canvasRef.current) canvasRef.current.style.cursor = (onCamera || onPC || onLibrary || onWhiteboard || onClock || onPhone || onSofa || onServer || id !== null || catNpcId !== null || onPhoto || onHeatmap) ? 'pointer' : 'default'
    }
  }
    ...
    if (!editor.isEditMode) {
      ...
      if (e.button === 0) {
        ...
        if (clickedCamera) {
          ...
          // Click on the cat NPC — toggle sprint/rage mode
        ...
        {showPhonePanel && !isEditMode && (() => {
          ...
          return (
            <div className={modalOverlayClass} onClick={() => setShowPhonePanel(false)}>
              <div className={modalPanelClass("w-80")} onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between mb-3">
                  <span className="font-semibold text-[var(--text)]">📱 Hermes Latest</span>
