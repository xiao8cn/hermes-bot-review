...
import { HERMES_HOME, getHermesProfileSessionsDir, listHermesProfiles } from '@/lib/hermes-paths'
...
async function loadAgentList(config: any): Promise<AgentConfigEntry[]> {
  ...
  return listHermesProfiles().map((id) => ({ id }))
}
...
function resolveCronStorePath(config: any): string {
  ...
  if (!raw) return path.join(HERMES_HOME, 'cron', 'jobs.json')
...
export async function GET() {
  ...
  try {
    ...
    const agentList = await loadAgentList(config)
    if (agentList.length > 0) {
      ...
      for (const agent of agentList) {
        let lastActive = 0
        const agentSessionsDir = getHermesProfileSessionsDir(agent.id)
        ...
        if (state !== 'offline' && existsSync(agentSessionsDir)) {
